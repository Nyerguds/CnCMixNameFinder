    ...
    public partial class InputBox: Form
    {
        ...
        private void textBoxText_TextChanged(Object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int firstChange;
                for (firstChange = 0; firstChange < length; firstChange++)
                {
                    if (input[firstChange] != output[firstChange])
                    {
                        break;
                    }
                }
                Boolean wasFiltered = input.Length != output.Length || firstChange < length;
                if (!wasFiltered)
                    return;
                // Update text field
                this.textBoxText.Text = output;
                this.textBoxText.SelectionStart = Math.Max(0, Math.Min(firstChange, this.textBoxText.Text.Length));
